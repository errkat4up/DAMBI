...
| operator runbook | `registryV2/docs/REGISTRY_RUNBOOK.md` (day-2 ops, incident response, rotation, rollback) |

---

## 10. Production registry project

Prod runs in a **dedicated, minimal-IAM GCP project**, separate from any application
workload: the bucket and the KMS key together form the supply-chain signing trust-root,
and broad `editor` bindings on a shared project are a poor home for one.
...
- **Custom domain**: not used for the registry proxy — the `*.run.app` host is the stable
  endpoint, baked into the build via DefinePlugin and never user-entered.
- **Extension build**: `.env` targets the registry URL + `PINNED_BUNDLE_PUBLIC_KEY`, and
  release artifacts require `DAMBI_REQUIRE_BUNDLE_SIGNATURE=true` (gated on the Tier-B
  coverage gate + Tier-D monitoring).

### Rollback
Signature coverage gaps fail closed: production builds keep
`DAMBI_REQUIRE_BUNDLE_SIGNATURE` **on**, so a bundle the proxy cannot serve a valid
signature for is never installed. Data rollback is the bucket-version restore in §7.
