...
**DEC-06c·DEC-06 현재 상태:** 구현·정적 검토·사용자 검증 완료. [확정 계약](#dec-06c--확정-계약과-실행-상태)을 유지하며 실행 근거는 [README 검증 기록](../../fixtures/decoder-policy/README.md#dec-06c--사용자-검증-완료-dec-06-완료) 한 곳을 따른다. 아래 06a/06b 상세는 해당 단계의 기존 기록이다. DEC-07·SDK 소스 이관은 미착수다.
...
| [typed Single](/Users/spu/SDKdambi/DAMBI/crates/policy-engine-wasm/tests/declarative_v3_typed_data_install.rs:164), [EIP-2612](/Users/spu/SDKdambi/DAMBI/crates/policy-engine-wasm/tests/declarative_v3_typed_data_install.rs:303), [Rust Batch 상수 시험](/Users/spu/SDKdambi/DAMBI/crates/policy-engine-wasm/tests/declarative_v3_typed_data_install.rs:1080) | flat/tuple/array 해석이 이미 존재. Rust Batch 상수는 현재 on-disk manifest와 별개 | 실제 build 경계와 원문 요청의 필수 구조·정보 보존 검증 |
...
fixtures/decoder-policy/
  ...
  multicall-self.cases.json
  multicall-self.test.mjs        # DEC-06a 사용자 재실행 48/48·통합 514/514 통과
  multicall-call-array.cases.json
  multicall-call-array.test.mjs  # DEC-06b 사용자 통합 검증 완료; 실행 기록은 README
  multicall-limits.cases.json
  multicall-limits.test.mjs     # DEC-06c 사용자 검증 완료
```
...
| --- | --- | --- | --- |
| **A — 현재 baseline + 교정 설계 (사용자 확정)** | 실제 v3 Single 연결/진단 85개와 이 설계를 유지하고 사용자 실행 검증 | 실제 원본 연결과 현행 동작을 확인. nonce 모델·fallback·선언 폭·시간·v4 Permit2 미지원은 명시적 미결 | **사용자 선택 완료.** Single 85/85·당시 통합 358/358, Batch 108/108·통합 466/466 사용자 검증 완료. Batch에도 같은 v3 연결/진단 범위 적용. 런타임 계약 교정은 별도 범위로 남김 |
...
#### DEC-05b 구현 — 사용자 실행 검증 완료

**코드 작성·정적 검토·사용자 실행 검증 완료.** 실제 [`permitBatch@1.0.0.json`](../../registryV2/manifests/uniswap/permit2/permitBatch@1.0.0.json)의 원본 바이트 SHA-256 `0x0343bc44659b2b1e7e184a39e7e17acd642ec52f2776adca963df4d8d3bd35ee`를 `selection.permit2_batch_manifest`에 고정했다. Single 검증 후 Batch fixture/test·최소 builder 옵션·script·네 문서를 작성했고, 사용자 저장 로그에서 요청 102개 + 구조 검사 6개 = **108/108**, 기존 358개 포함 통합 **466/466 통과**를 확인했다. A안이 계속 적용되며 Rust·worker·기존 Single fixture/test·기존 358개 기대값을 보존했다.

`buildRegistry(selection, { includePermit2Single: true, includePermit2Batch: true })`는 **approve+Single+Batch**를 선택한다. callkey 12개(각 4), typed index 8개(Single/Batch 각 4), selector index 0개, 네 체인 concrete Permit2 주소·원본 보존·typed/callkey 참조·JCS digest를 요구하고 실제 WASM에 설치한다. USDC permit은 이 Batch 시험 조합에 포함하지 않는다. `includePermit2Batch` 기본값은 false이며 명시 선택 때만 `permit2BatchSource`를 반환한다. 기존 선택의 기본값·반환·hash 검증·실패 시 임시 정리를 보존한다.

작성한 Batch 검사는 서로 다른 token/amount/expiration/nonce의 복수 원소, 순서·역순·중복 token 유지, 공통 spender/sigDeadline, 객체 key 재배열·ABI positional 변환, 첫째/둘째 필수 필드 누락/null/불량, empty/비배열/원소 타입·64/65 경계와 동일 프로세스의 Single/Batch primary type 구분이다. Permit2 verifying contract·underlying token·spender·submitter를 구분하고 signed message에 owner를 추가하지 않는다. 수량과 경계 기대값은 decimal string·BigInt로 독립 검산하며 내부 배열 호환·nonce fallback·선언 폭·큰 시간은 정상 EIP-712 객체와 별도 진단이다.

출력은 **최상위 Action 1개/외부 meta 1개 아래 Multicall의 자식 ActionBody** 구조다. details 64개이면 child 64개를 요구하고 65개는 `build_array_emit_failed`와 `exceeding max_elements=64`로 전체 실패한다. empty는 `{domain:"unknown",target:Permit2,chain:"eip155:1",calldata:"",value:"0x0"}`를 반환하는 기존 Unknown 경로다. details 누락/null/비배열과 일부 원소 token/amount/expiration 오류는 `build_array_emit_failed`, `data:null`인 전체 실패이며 정상 child를 부분 반환하지 않는다. 오류 envelope에 현재 없는 element path/index를 만들지 않는다. nonce만 누락/null/파싱 실패한 원소는 zero tuple로 성공할 수 있으므로 legacy diagnostic으로 분리한다. 현행 64/65 오류·empty Unknown을 관찰하므로 새 한도 계약을 도입하거나 재승인 질문할 필요가 없다.

원소별 nonce는 signed 값에서 word/bit로 분해되고 source `nonceBitmap(address,uint256)`·`permit2_nonce_bitmap`, ttl 12, synced_at=submitted_at을 가진다. 외부 조회나 zero stub으로 설명하지 않는다. shared sigDeadline이 모든 child/외부 meta에 동일하게 적용되는 정상 사례와 큰 수치의 JS 정밀도·u64 포화/meta 0 진단을 구분한다. v4 Batch는 설치 후에도 `unsupported_typed_data_contract`이며 상세 types 검증 전에 지원 범위 밖으로 반환한다. v3 retry는 없다. nonce 모델/범위/시간/v4 교정은 위 B/C의 미구현 제안으로 유지한다.

정확한 사례 분류·오류 메시지·현재 한계는 [Batch coverage](../../fixtures/decoder-policy/coverage.md#dec-05b--작성한-검사와-미결-계약), 사용자 실행은 [README](../../fixtures/decoder-policy/README.md#dec-05b-사용자-실행-기록--저장-로그-확인)의 **Batch 개별 → 기존 Single 포함 통합**을 따른다. 검증된 JS/WASM 쌍을 재사용하고 Single 개별을 별도로 재실행하지 않는다. Batch 실행 통과 수·resolved digest·실행 HEAD/시각을 위 저장 로그 기록에 반영했다. 완료 기록을 위한 재시험·재빌드는 필요하지 않다.

**완료 조건과 현재 한계:** Single과 Batch의 코드 작성·사용자 실행 결과를 각각 기록한다. Single과 Batch 사용자 실행 검증을 모두 확인하여 **DEC-05는 A안 범위에서 완료**다. nonce·입력 범위/형식·시간·v4 교정은 별도 후속 항목으로 유지한다. 새 요청 79개(normal 19, routing_miss 6, input_error 12, emit_error 26, legacy_diagnostic 14, compatibility 2) + 구조 검사 6개인 85개를 작성했다. 기존 다섯 Node 시험 파일의 273개 기대값을 유지하고 실제 작성한 Single을 더한 당시 통합 358개가 사용자 실행에서 모두 통과했다. 외부 nonce 조회·서명 유효성·정책 판정·witness/SignatureTransfer 기능·venue·Core·SDK 소스 이관은 미구현이다. 현재 원본과 실제 WASM의 연결 성공을 full EIP-712 검증이나 SDK 독립화 완료로 확대하지 않는다.
...
| **06c 진단·한도** | **사용자 검증 완료.** self·Call[]·callback의 공유 재귀 문맥과 한도, 미해석 원문 Unknown·진단 및 소비자 전달을 [확정 계약](#dec-06c--확정-계약과-실행-상태)에 맞춰 확인. [실행 근거](../../fixtures/decoder-policy/README.md#dec-06c--사용자-검증-완료-dec-06-완료)를 따르며 Permit2 Batch의 기존 계약은 유지 |
...
**사용자 계약 확정, 구현·정적 검토·사용자 검증 완료. DEC-06 전체 완료.** 새 Rust의 Native 회귀·WASM 빌드, 실제 Registry/Node 회귀와 TS 소비자 전달 검증을 마쳤다. 실행 수치·로그는 [README 검증 기록](../../fixtures/decoder-policy/README.md#dec-06c--사용자-검증-완료-dec-06-완료)에만 기록한다. 아래 확정 계약과 범위 제한은 유지하며 DEC-07은 미착수다.
...
- 안정 코드는 `depth_limit`, `child_limit`, `node_limit`, `unregistered_call`, `short_calldata`, `route_not_applicable`, `uninterpreted_action`이다. 해석한 결과와 순서를 유지하고 미해석 호출은 기존 Unknown(target/calldata/value)으로 보존하며 진단·partial은 요청 문맥을 통해 루트까지 전달한다.
- callback 전체를 읽지 못하면 **selector 없는 callback 원문**을 Unknown 하나로 보존한다. target/value는 callback을 제공한 원본 호출의 문맥이며 내부 Call의 target/value를 추측하지 않는다. 자체 Action이 없는 `reenter_only`도 callback을 전개하고, 아무 body도 얻지 못하면 원본 호출의 Unknown과 `uninterpreted_action`을 남긴다. 직접 callback route는 최상위 Multicall Action/meta 하나로 묶고 기존 `reenter_callback`도 유지한다.
- 루트 깊이 **0**, self/Call[] 자식 진입 **+1**, callback 구간 **+1**과 그 안의 Call 자식 **+1**이다. 전역 허용 깊이는 **4**까지다. 각 manifest의 상한은 `min(상속 상한, 현재 깊이 + max_depth)`이며 새 진입으로 초기화하지 않는다(self 3·Call[] 4). 이는 기존 callback만 네 번 전개하던 의미와 구분하는 명시적 계약 변경이다.
- 각 배열은 앞 **64개**를 해석하고 이후 원소는 Unknown+`child_limit`로 남긴다. 요청 전체의 **256개** 해석 노드 예산에 루트·callback 구간을 포함한다. DFS 순서로 공유하고 차단된 노드는 예산을 소비하지 않는다. 판정 우선순위는 배열 index 64 이상 → 깊이 → 노드 수다. 노드 한도는 ABI 할당·Unknown tail 출력량의 상한이 아니다.
- 기존 JSON 입력 **4 MiB** 제한을 유지하며 새 입력량 제한은 추가하지 않는다. 읽은 구간의 malformed ABI·emit·필수 해석 오류는 기존 오류를 유지한다. 한도 때문에 읽지 않은 구간의 내부 오류를 추정하지 않는다. self 자식 value=0과 Call[] 원소별 value 의미는 유지한다.
- 소비자의 타입 및 route→audit 전달만 수정한다. TS bundle 발견/사전 설치 한도는 별도이며 미리 설치한 실제 WASM 시험으로 그 범위를 검증했다고 표시하지 않는다. UI·정책 판정·Core/RPC·서명·SDK 이관·DEC-07은 범위 밖이다.
...
| callback 깊이 한도에서 전개를 생략하는 분기 | DEC-06c에서 원문 Unknown+사유 보존을 구현하고 사용자 검증 완료. [실행 근거](../../fixtures/decoder-policy/README.md#dec-06c--사용자-검증-완료-dec-06-완료) |
...
| DEC-05a Permit2 Single | 사용자 실행 85/85·당시 통합 358/358 통과. A 범위 확정: v3 연결·기존 한계 진단·교정 설계. 런타임 교정은 별도 범위 |
| DEC-05b Batch | 사용자 실행 검증 완료. 실제 Batch v3 연결·진단 108/108·통합 466/466 통과. DEC-05 A안 범위 완료 |
...
04a 결과를 네 문서에 반영하고, 네 계약 답변에 따라 별도 v4 full-input DTO·strict validator·emit 연결·Rust/Node 회귀 시험을 작성했다. 04b 사용자 실행의 저장 로그를 직접 확인했다. Native 181개와 새 WASM 빌드, Node strict 168개·기존 typed 47개·통합 273개가 모두 통과하여 **DEC-04 전체 검증 완료**다. 에이전트가 빌드·시험을 재실행한 결과는 아니다. 기존 DEC-01/02/03 사용자 통과 기록(30/37/58)과 미제공 항목을 보존하고 04a 47/105 통과 결과를 네 문서에 반영했다. 에이전트는 빌드·시험·설치·Git 변경 명령을 실행하지 않았다. 현재 DEC-05a는 **사용자 실행 검증 완료**이며 사용자가 **A — 연결 시험·교정 설계만 마무리**를 선택해 이번 범위는 확정됐다. B/C 런타임 계약 교정은 별도 범위의 미구현 제안이다. DEC-05b도 사용자 실행 108/108·통합 466/466 통과를 확인하여 DEC-05 A안 범위는 검증 완료다. nonce·입력 범위/형식·시간·v4 교정과 SDK 소스·빌드 독립화는 후속 범위다.
