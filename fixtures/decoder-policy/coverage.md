...
**DEC-06c·DEC-06 검증 상태:** 구현·정적 검토·사용자 검증 완료. self·Call[]·callback이 요청별 깊이/노드 예산과 진단을 공유하며 미해석 구간은 Unknown으로 보존한다. 실행 근거는 [README 검증 기록](README.md#dec-06c--사용자-검증-완료-dec-06-완료)을 따른다. 아래 06a/06b는 기존 기준선의 범위 기록이다. DEC-07의 [인계 index](handoff-index.json)는 같은 지원 범위와 설치 조합을 연결하며 source-context·동적 발견/설치·서명 검증을 추가 지원하지 않는다.
