...
// Handler helpers return `Result<T, Response>`: the axum Response IS the error
// value, so it is ~128 bytes and trips `result_large_err` on newer clippy.
// Boxing it would add an allocation on every rejection path for no benefit.
