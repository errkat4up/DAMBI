...
async fn cors_rejects_unconfigured_origin() {
    let addr =
        spawn_server_with_origin_allowlist(vec!["https://errkat4up-dambi.duckdns.org"]).await;
    let res = reqwest::Client::new()
...
async fn cors_allows_configured_dashboard_origin() {
    ...
    let res = reqwest::Client::new()
        .request(reqwest::Method::OPTIONS, format!("http://{addr}/auth/me"))
        .header("origin", "https://errkat4up-dambi.duckdns.org")
        .header("access-control-request-method", "GET")
        .send()
        .await
        .unwrap();
...
async fn cors_omits_private_network_approval_when_disabled() {
    let addr = spawn_server_with_cors(vec!["https://errkat4up-dambi.duckdns.org"], false).await;
    let res = reqwest::Client::new()
        .request(reqwest::Method::OPTIONS, format!("http://{addr}/auth/me"))
        .header("origin", "https://errkat4up-dambi.duckdns.org")
        .header("access-control-request-method", "GET")
...
async fn cors_emits_private_network_approval_when_enabled() {
    let addr = spawn_server_with_cors(vec!["https://errkat4up-dambi.duckdns.org"], true).await;
    let res = reqwest::Client::new()
        .request(reqwest::Method::OPTIONS, format!("http://{addr}/auth/me"))
        .header("origin", "https://errkat4up-dambi.duckdns.org")
        .header("access-control-request-method", "GET")
