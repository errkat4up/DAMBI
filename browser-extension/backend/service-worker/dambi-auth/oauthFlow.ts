 ...
 */
/**
 * Open the auth window, preferring the NATIVE `chrome.identity` callback form.
 *
 * The `webextension-polyfill` promise wrapper fails here with Chrome's opaque
 * "Authorization page could not be loaded." while the identical URL opened via
 * `chrome.identity.launchWebAuthFlow(opts, cb)` completes the whole flow —
 * verified by hand against this exact deployment. Prefer the native callback
 * form and keep the polyfill only as the Firefox / non-chrome fallback.
 */
async function launchWebAuthFlowCompat(
  polyfillIdentity: { launchWebAuthFlow(opts: { url: string; interactive: boolean }): Promise<string> },
  url: string,
): Promise<string> {
  const native = (globalThis as { chrome?: { identity?: { launchWebAuthFlow?: unknown } } }).chrome
    ?.identity?.launchWebAuthFlow;
  if (typeof native !== "function") {
    return polyfillIdentity.launchWebAuthFlow({ url, interactive: true });
  }
  return new Promise<string>((resolve, reject) => {
    (native as (
      ...
      (redirectUrl?: string) => {
        ...
        if (!redirectUrl) {
          reject(new Error("launchWebAuthFlow returned no redirect URL"));
          return;
        }
        resolve(redirectUrl);
      },
...
export async function startGoogleLogin(): Promise<{ access: string; refresh: string | null }> {
  ...
  console.log("[dambi] OAuth authorization URL:", url);
  ...
  try {
    redirectUrl = await launchWebAuthFlowCompat(identity, url);
  } catch (err) {
