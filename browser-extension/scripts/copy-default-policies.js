...
// The shared Day-1 source owns the order, Cedar, and required ManifestV2.
// Missing or invalid source files fail the build instead of emitting an empty
// set or silently synthesizing an empty-trigger manifest.
function copyDefaultPoliciesV2() {
  const destPath = path.join(DEST, "policy-set-v2.json");
  const policySet = loadPolicyBundle(REPO_ROOT);
  fs.writeFileSync(destPath, serializePolicyBundle(policySet));
  console.log(
